 ...
 *
 * Retries failed webhook deliveries up to 2 times with exponential
 * backoff. Errors during retries are caught and logged to prevent
 ...
 *
 * On failure, retries up to 2 additional times with exponential backoff.
 * All retry errors are caught to prevent unhandled promise rejections.
 */
    ...
    if (event.attempts < 3) {
      setTimeout(() => {
        deliverWebhook(url, event).catch((retryErr) => {
          console.error(
            `Webhook ${event.id} retry ${event.attempts} failed:`,
            retryErr
          );
        });
      }, 1000 * event.attempts);
