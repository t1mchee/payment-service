 ...
 * the customer and null-checks the result before accessing
 * .paymentMethodId.
 */
 ...
 * the case where the cache entry expired between the first call and
 * the retry.
 */
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        ...
        // The cache entry may have expired between the first fetch and
        // the retry, so getCustomer() can return null.
