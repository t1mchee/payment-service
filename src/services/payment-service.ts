 ...
 *
 * Handles charge creation with retry logic. The retry path
 * re-fetches the customer from cache and validates the result
 * before accessing .paymentMethodId.
 */
 ...
 * customer to get a fresh payment method and validates that the
 * customer still exists before accessing payment details.
 */
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        ...
        // and the retry, so we must null-check the result.
