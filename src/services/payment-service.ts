 ...
 * the customer from cache before each retry attempt, with a null check
 * to handle cache TTL expiry.
 */
 ...
 * customer to get a fresh payment method and null-checks the result
 * to handle the case where the cache entry expired between the
 * first call and the retry.
 */
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        ...
        // in case the cache entry was updated.
