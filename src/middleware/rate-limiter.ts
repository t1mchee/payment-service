...
/**
 * Remove entries from the store that have no timestamps within the current window.
 */
function cleanupExpiredEntries(now: number): void {
  for (const [key, entry] of rateLimitStore) {
    if (entry.timestamps.length === 0 || entry.timestamps.every((ts) => now - ts >= WINDOW_MS)) {
