...
describe("Payment Service - Null Check Fix Verification", () => {
  test("verifies the null-check fix in retry path", () => {
    // After fixing the bug, the retry path in createCharge() should:
    // 1. NOT use non-null assertion (retryCustomer!) on the re-fetched customer
    // 2. Properly null-check retryCustomer before accessing .paymentMethodId
