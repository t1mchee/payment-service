...
describe("Payment Service - Null Check Fix Verification", () => {
  test("verifies the null-check fix in retry path", () => {
    // Verify that the retry path properly null-checks the re-fetched customer.
    // Previously, the code used retryCustomer!.paymentMethodId without a null
    // check, causing TypeError when the cache entry expired between retries.
